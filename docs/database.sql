...
CREATE SCHEMA IF NOT EXISTS `zf2master` DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ;
USE `zf2master` ;

-- -----------------------------------------------------
-- Table `zf2master`.`seg_usuarios`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_usuarios` (
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_modulos`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_modulos` (
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_perfis`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_perfis` (
  ...
  CONSTRAINT `fk_seg_perfis_seg_modulos1`
    ...
    REFERENCES `zf2master`.`seg_modulos` (`mod_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_categorias_recursos`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_categorias_recursos` (
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_recursos`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_recursos` (
  ...
  CONSTRAINT `fk_seg_recursos_seg_categorias_recursos1`
    ...
    REFERENCES `zf2master`.`seg_categorias_recursos` (`ctr_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_permissoes`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_permissoes` (
  ...
  CONSTRAINT `fk_seg_permissoes_seg_recursos1`
    ...
    REFERENCES `zf2master`.`seg_recursos` (`rcs_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_perfis_usuarios`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_perfis_usuarios` (
  ...
  CONSTRAINT `fk_seg_usuarios_has_seg_perfis_seg_usuarios1`
    ...
    REFERENCES `zf2master`.`seg_usuarios` (`usr_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_seg_usuarios_has_seg_perfis_seg_perfis1`
    FOREIGN KEY (`pru_prf_id` )
    REFERENCES `zf2master`.`seg_perfis` (`prf_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `zf2master`.`seg_perfis_permissoes`
-- -----------------------------------------------------
CREATE  TABLE IF NOT EXISTS `zf2master`.`seg_perfis_permissoes` (
  ...
  CONSTRAINT `fk_seg_perfis_has_seg_permissoes_seg_permissoes1`
    ...
    REFERENCES `zf2master`.`seg_permissoes` (`prm_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Data for table `zf2master`.`seg_usuarios`
-- -----------------------------------------------------
START TRANSACTION;
USE `zf2master`;
INSERT INTO `zf2master`.`seg_usuarios` (`usr_id`, `usr_nome`, `usr_email`, `usr_telefone`, `usr_usuario`, `usr_senha`, `usr_ativo`) VALUES (1, 'Administrador', 'admin@gestor.uemanet.uema.br', '', 'admin', '$2y$14$VVypr584OQBTum6xZrZwbOyfMZckzHgBk7pmMKaywLOnVNu4pvGDq', 1);

COMMIT;

-- -----------------------------------------------------
-- Data for table `zf2master`.`seg_modulos`
-- -----------------------------------------------------
START TRANSACTION;
USE `zf2master`;
INSERT INTO `zf2master`.`seg_modulos` (`mod_id`, `mod_nome`, `mod_descricao`, `mod_icone`, `mod_ativo`) VALUES (1, 'Admin', 'Módulo de administração do portal', '', 1);

COMMIT;

-- -----------------------------------------------------
-- Data for table `zf2master`.`seg_perfis`
-- -----------------------------------------------------
START TRANSACTION;
USE `zf2master`;
INSERT INTO `zf2master`.`seg_perfis` (`prf_id`, `prf_mod_id`, `prf_nome`, `prf_descricao`) VALUES (1, 1, 'Administrador', 'Perfil de Administrador');

COMMIT;

-- -----------------------------------------------------
-- Data for table `zf2master`.`seg_perfis_usuarios`
-- -----------------------------------------------------
START TRANSACTION;
USE `zf2master`;
INSERT INTO `zf2master`.`seg_perfis_usuarios` (`pru_prf_id`, `pru_usr_id`) VALUES (1, 1);
